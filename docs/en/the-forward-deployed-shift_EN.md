...
```mermaid
graph LR
    A["Idea / PoC<br/>(anyone can reach)"] --> B["Working demo<br/>20% of the work"]
    B --> C{"Valley of Death<br/>──────<br/>integration, permissions, regulation,<br/>legacy, organizational politics<br/>80% of the work"}
    C -->|"95% die<br/>here"| D["Abandoned / shelved"]
    C -->|"only 5%<br/>cross"| E["Production<br/>= outcome deployment"]
...
```mermaid
graph TD
    A["Once: BUILD was scarce<br/>──────<br/>those who could build = valuable"] --> B["Industrialization by AI<br/>──────<br/>anyone can build via natural language<br/>(vibe coding / Claude Code)"]
    B --> C["Commoditization of BUILD<br/>──────<br/>'being able to build' is no differentiator<br/>marginal cost toward zero"]
    C --> D["Value moves to both ends<br/>──────<br/>① what to build (judgment)<br/>② can it be turned into an outcome (deployment)"]
    D --> E["Outcome deployment = the front line of ②<br/>──────<br/>the theme of this book"]
...
```mermaid
graph LR
    subgraph "Can be synthesized (AI's domain)"
        A["learned secondhand information<br/>general patterns<br/>draft hypotheses<br/>reproduction of known domains"]
    end
    subgraph "Cannot be synthesized (the field's domain)"
        B["unspoken constraints<br/>organizational politics<br/>tacit workflows<br/>unexpected discoveries<br/>emotion, irrationality, hesitation"]
    end
    A -.->|"deviates 13–23 pt<br/>on unknown questions"| B
    B -->|"can only be grasped<br/>by discovery in the field"| C["first-hand information<br/>= the only scarce resource"]
...
```mermaid
graph TD
    A["Build a bespoke solution at the customer's field<br/>(Deploy)"] --> B["Learn what the common problems are<br/>(Learn)"]
    B --> C["Abstract into general-purpose primitives<br/>(Codify)"]
    C --> D["Return to the core product<br/>(Automate)"]
    D --> E["The next customer gets it for free<br/>= compounding asset"]
    E -.->|"accelerates further<br/>on the next engagement"| A
    F["Zero return = the trap of contract work<br/>consulting shop in disguise"]
    A -.->|"if not even one feature returns<br/>within 90 days"| F
...
```mermaid
graph LR
    A["The first 10%<br/>──────<br/>what to solve in the field<br/>discovery, first-hand information<br/>【human, cannot be synthesized】"] --> B["The middle 80%<br/>──────<br/>research, analysis, implementation<br/>prototyping<br/>【orchestrate AI】"]
    B --> C["The last 10%<br/>──────<br/>the production-shipping judgment<br/>responsibility for the outcome<br/>【human, cannot be substituted】"]
...
![Practical steps to run outcome deployment in the field (D&V in practice)](../../assets/depth-and-velocity.png)
...
```mermaid
graph LR
    A["General-purpose AI model<br/>(anyone can use)"] --> B{"Outcome deployment<br/>──────<br/>connection to field data<br/>building evals<br/>stitching into operations"}
    B --> C["John Deere<br/>herbicide reduced ~50%<br/>31M gallons saved"]
    B --> D["BBVA<br/>120,000 people in 25 countries<br/>~3 hrs/week saved"]
    B --> E["Morgan Stanley<br/>document access<br/>20% → 80%"]
